...
struct LEDModule {
    ...
    uint32_t color; // 24bit data, that holds the RGB Value -> together with 50bits 'break' we get the length of the pwmData
    uint16_t LEDModule_pwmData[74]; // array of pulse width information, the length of the high for each duty cycle per bit: [10, 10, 90, 10, 90, 90, ...]
};
